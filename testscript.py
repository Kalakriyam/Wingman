...
import orjson
import sqlite3
from mini_and_port import fetch_tts_mp3, decode_mp3_to_pcm, play_pcm_blocking
...
from mini_and_port import play_pcm_blocking, fetch_tts_mp3, decode_mp3_to_pcm
...
#                 if not mp3_bytes:
#                     raise RuntimeError("TTS gave 0 bytes")

#                 # decode to signed-16 PCM
...
#                 pcm   = np.asarray(decoded.samples, dtype=np.int16)   # shape (frames, chans)
#                 rate  = int(decoded.sample_rate)
#                 chan  = int(decoded.nchannels)

#                 # optional trim-and-fade (kept minimal)
#                 if DEFAULT_TRIM_MS:
#                     pcm = pcm[:-int(rate * DEFAULT_TRIM_MS / 1000)]
#                 if DEFAULT_FADE_MS and pcm.size:
#                     n = min(int(rate * DEFAULT_FADE_MS / 1000), pcm.shape[0])
#                     if n:
#                         fade = np.linspace(1.0, 0.0, n, dtype=np.float32)
#                         if pcm.ndim == 1:                       # 1-channel
#                             pcm[-n:] = (pcm[-n:] * fade).astype(np.int16)
#                         else:                                   # 2-plus channels
#                             pcm[-n:] = (pcm[-n:] * fade[:, None]).astype(np.int16)

#                 # pack for the player
#                 audio_segments[order] = {
#                     "bytes": pcm.tobytes(),   # ⚠ bytes, **not** ndarray
#                     "rate":  rate,
#                     "chan":  chan,
#                 }
#                 if order == 0:
...
async def tts_request(sentence, order):
    ...
    try:
        mp3_bytes = await fetch_tts_mp3(sentence)
        if not mp3_bytes:
            raise RuntimeError("TTS gave 0 bytes")
        pcm_bytes, sr = await asyncio.to_thread(decode_mp3_to_pcm, mp3_bytes)

        audio_segments[order] = pcm_bytes
    ...
    while True:
        while audio_segments.get(audio_order) is not None:
            ...
            if content == "MIDI_COMMAND":
                ...
                await asyncio.to_thread(play_pcm_blocking, content)
